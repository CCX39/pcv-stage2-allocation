...
Phase 2B.1 已完成通用传输版本候选迁移。Phase 2B.2 只整理中文文档和英文 Markdown 镜像，不改变本契约的技术内容。
...
MVP 当前未实现真实 Longdress 分块或 PLY/DRC artifact 读取，未接入 data-prep artifact root，未生成 frame 1051 正式输入，未测 target-side `D`，未构建 DRC-aware 或 format-aware `q`，未实现 target-aware lookup、Pareto pruning、baseline、batch runner、plotting 或播放器接入。

下一阶段 Phase 2B.3 是真实候选元数据只读桥接；Phase 2B.4 才是 frame 1051 求解器行为验证。两者均尚未开始。
