...
Status: Phase 0B draft. This document explains the JSON Schema drafts added for the Stage2 MVP. The schemas describe data format only. They do not implement validation code, a solver, test fixtures, or experiments.
...
The schemas are intended to support review, later validation, and reproducible experiments. They are not a claim that Stage2 allocation can already run.
...
D0-4 remains `DRAFT`. The vocabulary is sufficient for Phase 0B schema drafting, but it is not a final field-level provenance design.
...
## 11. Not Implemented In Phase 0B

Phase 0B does not implement:
...
Later phases can use these schemas to validate hand-written inputs, lookup profiles, and solver outputs. After fixture design begins, the schemas may be refined based on real sample data, but any semantic change to D0-1, D0-2, or D0-3 must be recorded in the decision log.
