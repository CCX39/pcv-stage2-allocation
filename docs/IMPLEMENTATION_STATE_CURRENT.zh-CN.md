...
本文是 `pcv-stage2-allocation` 的阶段1G接手说明，用来记录截至阶段1G的实现状态，方便新的对话或人工审查者快速继续工作。
...
目前尚未实现 runtime 精确 MCKP 求解、通用 validator、实验运行器或播放器集成。
...
下一步不建议在未审查 local-upgrade 审计输出和 small-instance oracle 检查前直接进入实验。较稳妥的选择是：
