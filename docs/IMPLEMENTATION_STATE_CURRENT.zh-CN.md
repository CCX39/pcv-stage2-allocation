...
本文是 `pcv-stage2-allocation` 的阶段1E接手说明，用来记录截至阶段1E的实现状态，方便新的对话或人工审查者快速继续工作。
...
目前尚未实现 local upgrade、精确 MCKP 求解、通用 validator、实验运行器或播放器集成。
...
fixture 防线脚本继续保留独立校验路径，用于检查 Schema 草案和 handcheck JSON 文件；pytest 单独检查模型层、lookup cap 预处理、`B_min_feasible`、阶段1B固定 `lambda` 内核、阶段1C括区间 trace 内核、阶段1D二分搜索内核、阶段1E结构化 solver result 和 handcheck 预期值。
...
下一步不建议在未审查结构化 solver result 前直接进入实验。较稳妥的选择是：
