...
本文是 `pcv-stage2-allocation` 的阶段1D接手说明，用来记录截至阶段1D的实现状态，方便新的对话或人工审查者快速继续工作。
...
fixture 防线脚本继续保留独立校验路径，用于检查 Schema 草案和 handcheck JSON 文件；pytest 单独检查模型层、lookup cap 预处理、`B_min_feasible`、阶段1B固定 `lambda` 内核、阶段1C括区间 trace 内核、阶段1D二分搜索内核和 handcheck 预期值。两者都不是完整求解器。
...
下一步不建议在未审查二分搜索内核前直接进入 local upgrade 或实验。较稳妥的选择是：
...
6. 再读 `docs/schema_contract.zh-CN.md`。
7. 再看 `tests/fixtures/handcheck_3x3/hand_calculation.zh-CN.md`。
8. 安装依赖后运行 `python -m pytest` 和 `python scripts/validate_handcheck_fixtures.py`。
9. 当前不要修改 D0-1、D0-2、D0-3 的冻结语义。
10. 不要把 `handcheck_3x3` fixture 当成真实 Longdress 实验。
